...
module "eks_main" {
  ...
  aws_auth_users = [
    {
      userarn  = "arn:aws:iam::${var.aws_account_id}:user/ali.yousefian"
      username = "ali.yousefian"
      groups   = ["system:masters"]
    },
    {
      userarn  = "arn:aws:iam::${var.aws_account_id}:user/hosein.beigi"
      username = "hosein.beigi"
      groups   = ["system:masters"]
    },
    {
      userarn  = "arn:aws:iam::${var.aws_account_id}:user/m.fallahi"
      username = "m.fallahi"
      groups   = ["system:masters"]
    },
    {
      userarn  = "arn:aws:iam::${var.aws_account_id}:user/mehdi.azimi"
      username = "mehdi.azimi"
      groups   = ["system:masters"]
    }
  ]

  kms_key_administrators = ["arn:aws:iam::${var.aws_account_id}:user/ali.yousefian", "arn:aws:iam::${var.aws_account_id}:user/hosein.beigi", "arn:aws:iam::${var.aws_account_id}:user/m.fallahi", "arn:aws:iam::${var.aws_account_id}:user/mehdi.azimi"]
...
module "iam_group_eks_main_admin" {
  ...
  create_group                      = true
  group_users                       = var.eks_main_admins
...
# EKS - main - calico
resource "helm_release" "eks_main_calico" {
...
resource "null_resource" "eks_main_calico_clusterrole" {
  provisioner "local-exec" {
    command = "kubectl --kubeconfig ~/.kube/${var.project}-eks-main-${var.environment} patch clusterrole aws-node --type='json' -p='[{\"op\": \"add\", \"path\": \"/rules/-\", \"value\": {\"apiGroups\": [\"\"], \"resources\": [\"pods\"], \"verbs\": [\"patch\"]}}]'"
  }
...
resource "null_resource" "eks_main_calico_annotate_pod_ip" {
  provisioner "local-exec" {
    command = "kubectl --kubeconfig ~/.kube/${var.project}-eks-main-${var.environment} set env daemonset aws-node -n kube-system ANNOTATE_POD_IP=true"
  }
...
# EKS - main - ingress-nginx
resource "helm_release" "eks_main_ingress_nginx" {
  chart            = "ingress-nginx"
  ...
  set {
    ...
    value = join("\\,", module.vpc_main.public_subnets)
  }
  ...
  set {
    ...
    value = var.eks_main_ingress_nginx.min_replicas
  }

  set {
    name  = "controller.autoscaling.maxReplicas"
    value = var.eks_main_ingress_nginx.max_replicas
  }

  set {
    name  = "controller.autoscaling.targetMemoryUtilizationPercentage"
    value = var.eks_main_ingress_nginx.target_memory_utilization
  }

  set {
    name  = "controller.resources.requests.cpu"
    value = var.eks_main_ingress_nginx.req_cpu
  }

  set {
    name  = "controller.resources.requests.memory"
    value = var.eks_main_ingress_nginx.req_mem
  }
  ...
  set {
    name  = "controller.config.global-rate-limit-memcached-host"
    value = "ingress-nginx-memcached.ingress-nginx.svc.cluster.local"
  }

  set {
    name  = "controller.config.global-rate-limit-memcached-port"
    value = "11211"
  }

  set {
    name  = "controller.config.global-rate-limit-memcached-connect-timeout"
    value = "20ms"
  }

  set {
    name  = "controller.config.global-rate-limit-status-code"
    value = "429"
  }

  set {
    name  = "controller.config.log-format-upstream"
    value = "{\"time\": \"$time_iso8601\", \"remote_addr\": \"$proxy_protocol_addr\",\"x_forward_for\": \"$proxy_add_x_forwarded_for\", \"request_id\": \"$req_id\", \"remote_user\":\"$remote_user\", \"bytes_sent\": $bytes_sent, \"request_time\": $request_time, \"status\":$status, \"host\": \"$host\", \"request_proto\": \"$server_protocol\", \"path\": \"$uri\",\"request_path\": \"$request_uri\", \"request_query\": \"$args\", \"request_length\": $request_length,\"duration\": $request_time,\"method\": \"$request_method\", \"http_referrer\": \"$http_referer\",\"http_user_agent\": \"$http_user_agent\", \"scheme\": \"$scheme\" }"
  }

  set {
    name  = "controller.config.http-snippet"
    value = "proxy_cache_path /tmp/nginx_my_cache levels=1:2 keys_zone=main-cache:10m use_temp_path=off max_size=100m inactive=48h;"
  }
...
# EKS - main - cloudwatch grafana
module "irsa_eks_main_cloudwatch_grafana" {
  source  = "terraform-aws-modules/iam/aws//modules/iam-role-for-service-accounts-eks"
  version = "~> 5.20.0"

  role_name = "eks-main-cloudwatch-grafana-${var.environment}"

  role_policy_arns = {
    policy = "arn:aws:iam::aws:policy/service-role/AmazonGrafanaCloudWatchAccess"
  }
