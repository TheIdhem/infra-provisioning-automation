...
# EC2 - insight
module "security_group_ec2_insight" {
  count = var.environment == "dev" ? 0 : 1
  source  = "terraform-aws-modules/security-group/aws"
  version = "~> 5.0.0"

  name   = "ec2-insight-${var.environment}"
  vpc_id = module.vpc_main.vpc_id
...
module "ec2_insight" {
  ...
  availability_zone      = element(module.vpc_main.azs, 0)
  subnet_id              = element(module.vpc_main.public_subnets, 0)
  vpc_security_group_ids = [module.security_group_ec2_insight[0].security_group_id]
...
resource "aws_eip" "ec2_insight" {
  count = var.environment == "dev" ? 0 : 1
  instance = module.ec2_insight[0].id
  domain   = "vpc"
...
resource "aws_route53_record" "ec2_insight" {
  count = var.environment == "dev" ? 0 : 1
  zone_id = data.aws_route53_zone.public_domain.zone_id
  name    = format("insight.infra.%s", data.aws_route53_zone.public_domain.name)
  type    = "A"
  records = [aws_eip.ec2_insight[0].public_ip]
  ttl     = 60
}
